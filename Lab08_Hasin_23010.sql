...
select Dept_Name from department where Dept_ID in (select Dept_ID from Employee group by Dept_ID having (select avg(cast(Salary as int)) from Employee) > avg(cast(Salary as int)));

select Dept_Name from department where Dept_ID = any (select Dept_ID from Employee group by Dept_ID having (select avg(cast(Salary as int)) from Employee) > avg(cast(Salary as int)));

select ID, Name, Salary from Employee where Salary > (Select Salary from Employee where Name='Asad')

select * from Employee where ID=any (select ID from Employee where ID=6 or ID=9)

delete from employee where cast(Salary as int) in (select Salary from Employee where cast(Salary as int) > 25000);
